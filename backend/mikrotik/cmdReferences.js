...
const PPP = {
  ...
  /**
   * Remove a PPP secret by ID.
   * @param {string} id - Secret .id.
   * @returns {string[]} Command array.
   */
  removeSecret: (id) => {
    if (!id) throw new Error("ID required for removeSecret");
    return ["/ppp/secret/remove", `=.id=${id}`];
  },
   ...
   * @returns {string[]} Command array.
   */
  getActivePPPoE: ["/ppp/active/print", "?service=pppoe"],
};
...
// ===================== EXPORTS =====================
export { SYSTEM, INTERFACE, IP, QUEUE, PPP, TOOLS, WIRELESS };
